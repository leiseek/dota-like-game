...
function drawCrystalEventEffect(context: CanvasRenderingContext2D, effect: CrystalEventEffect, nowMs: number, stackIndex: number): void {
...
function paletteForTone(tone: CrystalEventTone): Readonly<{ backdrop: string; border: string; title: string; ring: string }> {
  switch (tone) {
    case "danger":
...
function drawCrystalIcon(context: CanvasRenderingContext2D, x: number, y: number, color: string, ratio: number): void {
