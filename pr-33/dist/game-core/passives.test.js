...
function pathEnemy(id, progress, health) {
...
function withRunningState(state) {
    return { ...state, status: "running", clock: { ...state.clock, paused: false } };
}
function withHeroProgression(state, heroId, heroLevel) {
...
test("damage-over-time status effects with source heroes are processed during running ticks", () => {
    ...
    const enemy = {
        ...
        statusEffects: [
            { type: "poison", remainingTicks: 3, damagePerTick: 1, sourceHeroId: "hero-1" },
            { type: "burn", remainingTicks: 3, damagePerTick: 1, sourceHeroId: "hero-1" },
        ],
    };
