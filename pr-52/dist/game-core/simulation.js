...
function gainHeroExperience(hero, experienceToAdd, level) {
...
function getXpPerKill(level, hero) {
...
function getSkillDamageAgainst(level, hero, target) {
...
function addPassiveOnHitEffects(mutations, hero, enemy, level) {
    if (hasPassiveKind(level, hero, "slow")) {
        addStatusEffect(mutations, enemy.id, { type: "slow", remainingTicks: msToTicks(DEFAULT_PASSIVE_SLOW_MS, level), speedMultiplier: 0.8, sourceHeroId: hero.id });
    }
...
function applyStatusEffectDamage(state, level) {
...
function getStatusSpeedMultiplier(enemy) {
    ...
    if (active.some((statusEffect) => statusEffect.type === "stun"))
        return 0;
...
function advanceEnemyAlongPath(enemy, level) {
    const direction = enemy.carryingCrystal || enemy.returningToStart ? -1 : 1;
...
function applyTowerCombat(state, level) {
    ...
    const heroes = state.heroes.map((hero) => {
        ...
        if (!config || cooledHero.attackCooldownMs > 0)
            return cooledHero;
        ...
        if (hasPassiveKind(level, cooledHero, "lightning-chain")) {
            ...
            if (chainTarget)
                addDamage(mutations, chainTarget.id, Math.round(config.attackDamage * 0.5));
        }
        if (hasPassiveKind(level, cooledHero, "cleave")) {
            for (const enemy of enemies) {
                if (enemy.id === target.id || distance(enemy.position, target.position) > DEFAULT_PASSIVE_CLEAVE_RADIUS)
                    continue;
        ...
        enemies = [...result.enemies];
        let nextHero = { ...cooledHero, attackCooldownMs: config.attackIntervalMs, targetEnemyId: target.id };
        if (result.killedEnemies.length > 0) {
            ...
            if (!killedCarrier)
                killedCarrier = result.killedEnemies.find((enemy) => enemy.carryingCrystal);
...
function applyAutoCastSkills(state, level) {
    let nextState = state;
    for (const hero of state.heroes) {
        const currentHero = nextState.heroes.find((candidate) => candidate.id === hero.id);
        if (!currentHero?.autoCastEnabled || currentHero.cooldownTicksRemaining > 0)
            continue;
...
function resolveCrystalAndBase(state, advancedEnemies, level) {
    ...
    for (const enemy of advancedEnemies) {
        ...
        if (!enemy.returningToStart && !enemy.carryingCrystal && enemy.progress >= 1 && enemy.pathIndex === lastPathSegmentIndex) {
            if (crystal.atBase) {
                const carrier = { ...enemy, progress: 1, carryingCrystal: true, returningToStart: true, ...(ancientPosition ? { position: ancientPosition } : {}) };
...
function advanceWave(state, level) {
    ...
    if (!waveConfig)
        return { enemies: preparedState.enemies, wave: preparedState.wave };
    let wave = { ...preparedState.wave, waveElapsedMs: preparedState.wave.waveElapsedMs + preparedState.clock.fixedDeltaMs };
