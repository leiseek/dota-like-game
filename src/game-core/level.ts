import type { HeroPassiveConfig, HeroProgressionConfig, LevelConfig } from "./types.js";

function passives(archetype: string, entries: readonly Omit<HeroPassiveConfig, "id">[]): readonly HeroPassiveConfig[] {
  return entries.map((entry) => ({ ...entry, id: `${archetype}:lv${entry.level}:${entry.kind}` }));
}

function progression(archetype: string, entries: readonly Omit<HeroPassiveConfig, "id">[]): HeroProgressionConfig {
  return {
    xpPerKill: 1,
